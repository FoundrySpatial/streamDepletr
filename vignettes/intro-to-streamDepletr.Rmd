...
The stream network and discharge data are included in the package (`stream_shp` and `discharge_df`, respectively). First, let's we'll define some of the properties of the well and the aquifer:
```{r}
# well properties
...
```{r}
rdll <- prep_reach_dist(wel_lon=wel_lon, wel_lat=wel_lat, stream_shp=stream_shp, reach_id="reach", stream_pt_spacing=1)
head(rdll)
```
...
closest_stream <- stream_shp@data$stream[stream_shp@data$reach==closest_reach]  # figure out if closest reach is on Dorn or Sixmile Creek
